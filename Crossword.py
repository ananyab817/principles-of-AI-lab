...
def main():
    ...
    for word in words:
        placed = False
        while not placed:
            x = random.randint(0, crossword.width - 1)
            y = random.randint(0, crossword.height - 1)
            direction = random.choice([(1, 0), (0, 1)])

            if crossword.check_fit(word, x, y, direction[0], direction[1]):
                crossword.place_word(word, x, y, direction[0], direction[1])
                placed = True
